...
for feature in layer:
    ...
    myplot.setproperty('PDATE', '2018-04-18')
    ...
    fdata = list()
    fdata.append({'date':'2018-05-16','value':39.3})
    fdata.append({'date':'2018-06-20','value':41.4})
    fdata.append({'date':'2018-07-05','value':53.1})
    fdata.append({'date':'2018-07-19','value':45.5})
    myplot.setproperty('FEAMN',fdata)
    cdata = list()
    cdata.append({'date':'2018-03-02','value':'Apply RoundUp (glyphosate)'})
    cdata.append({'date':'2018-04-11','value':'Apply Prowl (pendimethalin) and RoundUp (glyphosate)'})
    cdata.append({'date':'2018-05-22','value':'Apply RoundUp (glyphosate)'})
    cdata.append({'date':'2018-07-04','value':'Apply Carbine (flonicamid)'})
    cdata.append({'date':'2018-07-14','value':'Apply Transform (sulfoxaflor)'})
    cdata.append({'date':'2018-07-18','value':'Apply Carbine (flonicamid)'})
    cdata.append({'date':'2018-07-28','value':'Apply Transform (sulfoxaflor)'})
    cdata.append({'date':'2018-09-08','value':'Apply Transform (sulfoxaflor) and Admiral (pyriproxyfen)'})
    cdata.append({'date':'2018-09-15','value':'Apply Carbine (flonicamid)'})
    cdata.append({'date':'2018-09-28','value':'Apply Ginstar (diuron, thidiazuron) and CottonQuik (urea sulfate, ethephon)'})
    cdata.append({'date':'2018-10-19','value':'Apply Ginstar (diuron, thidiazuron) and CottonQuik (urea sulfate, ethephon)'})
    myplot.setproperty('CH_NOTES',cdata)
    tdata = list()
    tdata.append({'date':'2018-11-26','value':'Root pull and disk'})
    tdata.append({'date':'2018-11-28','value':'Rip'})
    tdata.append({'date':'2018-12-04','value':'Moldboard plow'})
    tdata.append({'date':'2018-12-05','value':'Disk'})
    tdata.append({'date':'2018-12-06','value':'Laser level'})
