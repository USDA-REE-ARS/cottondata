...
for feature in layer:
    ...
    myplot.setproperty('PDATE', '2019-04-18')
    ...
    fdata = list()
    fdata.append({'date':'2019-05-30','value':42.6})
    fdata.append({'date':'2019-06-21','value':42.6})
    fdata.append({'date':'2019-07-11','value':63.9})
    myplot.setproperty('FEAMN',fdata)
    cdata = list()
    cdata.append({'date':'2019-03-08','value':'Apply RoundUp (glyphosate)'})
    cdata.append({'date':'2019-04-08','value':'Apply Prowl (pendimethalin)'})
    cdata.append({'date':'2019-05-09','value':'Apply RoundUp (glyphosate)'})
    cdata.append({'date':'2019-10-04','value':'Apply CutOut (diuron, thidiazuron) and CottonQuik (urea sulfate, ethephon)'})
    cdata.append({'date':'2019-10-18','value':'Apply CutOut (diuron, thidiazuron) and CottonQuik (urea sulfate, ethephon)'})
    myplot.setproperty('CH_NOTES',cdata)
    tdata = list()
    tdata.append({'date':'2019-11','value':'Root pull, disk, rip, moldboard plow, disk and land plane'})
...
for feature in layer:
    ...
    rows = swc.loc[swc['TubeID'] == int(tb_label)]
