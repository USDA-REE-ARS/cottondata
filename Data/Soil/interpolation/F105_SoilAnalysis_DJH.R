...
pdf("./F105_SoilAnalysis_DJH.pdf")

#Read in soil data
soildf<-read.table(file="./F105_SoilAnalysis_DJH.csv",header=TRUE,sep=',')
        ...
        for (max.dist in seq(75,100,1)){ #Maximum distance on variogram
            for (numbins in seq(5,9,1)){ #Number of bins on variogram
                ...
                if (var %in% c('SLSND','SLSLT','SLCLY')){
                    ...
                    if (header=="SLWP1015"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00072, fix.nugget=TRUE)
                    } else if (header=="SLWP1045"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00118, fix.nugget=TRUE)
                    } else if (header=="SLWP1135"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00120, fix.nugget=TRUE)
                    } else if (header=="SLWP1165"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00084, fix.nugget=TRUE)
                    } else if (header=="SLFC1015"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00030, fix.nugget=TRUE)
                    } else if (header=="SLFC1075"){
                        fit.gdata<-variofit(var.gdata,nugget=0.00300, fix.nugget=TRUE)
                    } else if (header=="SLFC1165"){ #T
                        fit.gdata<-variofit(var.gdata,nugget=0.00230, fix.nugget=TRUE)
                    } else {
...
write.csv(outdf,file="./F105_SoilAnalysis_DJH_Interpolate.csv")
