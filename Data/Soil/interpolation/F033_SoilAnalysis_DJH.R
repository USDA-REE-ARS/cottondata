...
pdf("./F033_SoilAnalysis_DJH.pdf")

#Read in soil data
soildf<-read.table(file="./F033_SoilAnalysis_DJH.csv",header=TRUE,sep=',')
...
depths<-c(15,45,75,105,135,165,210)
        ...
        for (max.dist in seq(75,100,1)){ #Maximum distance on variogram
            for (numbins in seq(5,9,1)){ #Number of bins on variogram
                ...
                if (var %in% c('SLSND','SLSLT','SLCLY')){
                    ...
                    if (header=="SLCLY210"){
                        fit.gdata<-variofit(var.gdata,nugget=12.0, fix.nugget=TRUE, limits=limits)
                    } else if (header=="SLCLY165"){
                    ...
                    } else if (header=="SLSND015"){
                        fit.gdata<-variofit(var.gdata,nugget=3.0, fix.nugget=TRUE, limits=limits)
                    } else {
...
write.csv(outdf,file="./F033_SoilAnalysis_DJH_Interpolate.csv")
