...
shapefile = '../F033_SoilAnalysis_DJH.shp'
...
datafile = '../F033_SoilAnalysis_DJH.xlsx'
soildf = pd.read_excel(datafile,sheet_name='SoilDF')

newdf = pd.merge(soildf,spatialdata,on="Core",how='left')

mask = newdf['Core'].str.contains('dup',na=False)
newdfnodup = newdf[~mask]

newdfnodup.to_csv('F033_SoilAnalysis_DJH.csv',index=False)
